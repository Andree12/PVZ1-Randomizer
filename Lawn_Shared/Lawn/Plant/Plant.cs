...
namespace Lawn
{
    public/*internal*/ class Plant : GameObject
    {
        ...
        public void Fire(Zombie theTargetZombie, int theRow, PlantWeapon thePlantWeapon)
        {
            ...
            if (seedType == SeedType.Threepeater || seedType == SeedType.Peashooter || seedType == SeedType.Repeater || seedType == SeedType.Splitpea || seedType == SeedType.Gatlingpea)
            {
            ...
            if (seedType == SeedType.Cactus || seedType == SeedType.Cattail)
            {
                projectileType = ProjectileType.Spike;
                goto IL_157;
            }
            if (seedType == SeedType.Scaredyshroom || seedType == SeedType.Puffshroom || seedType == SeedType.Seashroom)
            {
        ...
        public TRect GetPlantAttackRect(PlantWeapon thePlantWeapon)
        {
            ...
            else
            {
                ...
                else if (seedType == SeedType.Sunshroom || seedType == SeedType.Tallnut || seedType == SeedType.GoldMagnet || seedType == SeedType.Seashroom)
                {
                    goto IL_27E;
                }
                else if (seedType == SeedType.Chomper)
                {
                    result = new TRect(mX + 80, mY, 40, mHeight);
                    return result;
                }
                else if (seedType == SeedType.Fumeshroom)
                {
                    result = new TRect(mX + 60, mY, 340, mHeight);
                    return result;
                }
                else if (seedType == SeedType.Squash)
                {
                    result = new TRect(mX + 20, mY, mWidth - 35, mHeight);
                    return result;
                }
                else if (seedType == SeedType.Tanglekelp)
                {
                    result = new TRect(mX, mY, mWidth, mHeight);
                    return result;
                }
                else if (seedType == SeedType.Spikeweed)
                {
                    goto IL_15B;
                }
                else if (seedType == SeedType.Torchwood)
                {
                    result = new TRect(mX + 50, mY, 30, mHeight);
                    return result;
                }
                else if (seedType == SeedType.Gloomshroom)
                {
                    result = new TRect(mX - 80, mY - 80, 240, 240);
                    return result;
                }
                else if (seedType == SeedType.Cattail)
                {
                    result = new TRect(-800, -600, 1600, 1200);
                    return result;
                }
                else if (seedType == SeedType.Leftpeater)
                {
